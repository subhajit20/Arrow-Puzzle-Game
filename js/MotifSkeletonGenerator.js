...
class MotifSkeletonGenerator {
    ...
    generate(motifAssignments, regionLayout, grid) {
        ...
        for (const { regionId, type, parameters } of motifAssignments) {
            ...
            const orderedPath = this._buildOrderedPath(type, parameters, bb, regionSet);
    ...
    _corridorSkeleton({ axis, width }, bb, regionSet) {
        ...
        if (axis === 'H') {
            ...
            for (let c = minC; c <= maxC; c++) {
                for (let dR = -(width - 1); dR <= (width - 1); dR++) {
                    const r = mid + dR;
            ...
            for (let r = minR; r <= maxR; r++) {
                for (let dC = -(width - 1); dC <= (width - 1); dC++) {
                    const c = mid + dC;
                    if (!regionSet.has(`${r},${c}`)) continue;
    ...
    _buildOrderedPath(type, parameters, bb, regionSet) {
        let candidates;
        switch (type) {
            case 'CORRIDOR':    candidates = this._corridorOrdered(parameters, bb);    break;
    ...
    // NESTED_RECT — concentric CW/CCW alternating rectangles
    _nestedRectOrdered(params, bb) {
        return this._rectNestedInOut(bb, 2);
    }
